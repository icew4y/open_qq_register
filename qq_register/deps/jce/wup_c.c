...
Int32 UniPacket_decode(UniPacket * pack, const char* buff, uint32_t len)
{
    ...
    int version = 0, cPacketType = 0, iMessageType = 0, iRequestId = 0;
    JString *sServantName, *sFuncName, *sBuffer;
    sServantName = JString_new();
    sFuncName = JString_new();
    sBuffer = JString_new();
    
	ret = JceInputStream_readShort(is, &version/*pack->iVersion*/, 1, true);     if (JCE_SUCCESS != ret) return ret;
    printf("version:%d\r\n", version);
	ret = JceInputStream_readChar(is, &cPacketType, 2, false);  if (JCE_SUCCESS != ret) return ret;
    printf("cPacketType:%d\r\n", cPacketType);
	ret = JceInputStream_readChar(is, &iMessageType, 3, false);if (JCE_SUCCESS != ret) return ret;
    printf("iMessageType:%d\r\n", iMessageType);
	ret = JceInputStream_readInt32(is, &iRequestId, 4, true);   if (JCE_SUCCESS != ret) return ret;
    printf("iRequestId:%d\r\n", iRequestId);
	ret = JceInputStream_readString(is, sServantName, 5, true); if (JCE_SUCCESS != ret) return ret;
    printf("sServantName:%s\r\n", sServantName->_data);
	ret = JceInputStream_readString(is, sFuncName, 6, true);    if (JCE_SUCCESS != ret) return ret;
    printf("sFuncName:%s\r\n", sFuncName->_data);
	ret = JceInputStream_readVectorChar (is, sBuffer, 7, true); if (JCE_SUCCESS != ret) return ret;
    printf("");
    ...
    UniAttribute pp;
    UniAttribute_init(&pp);
    UniAttribute_decode(&pp, sBuffer->_data, sBuffer->_len);


    JMapWrapper	* mRes = JMapWrapper_new("string", "map<string,list<char>>");
    WUP_getMap((const void *)&pp, "RespSummaryCard", mRes);
